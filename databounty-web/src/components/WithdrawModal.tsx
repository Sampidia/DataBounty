    ...
    <>
      ...
      {showInsufficientFundsPopup && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-200">
          <div className="relative w-full max-w-sm bg-[#031F51] border border-red-500/40 rounded-2xl shadow-2xl shadow-red-500/20 overflow-hidden text-white animate-in zoom-in-95 duration-200">
            {/* Top accent */}
            <div className="h-1.5 w-full bg-gradient-to-r from-red-500 via-rose-500 to-orange-400" />

            <div className="p-6 text-center space-y-4">
              {/* Icon */}
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-red-500/15 border border-red-500/30 text-red-400 mx-auto">
                <AlertTriangle className="w-8 h-8" />
              </div>

              <div className="space-y-1.5">
                <h3 className="text-lg font-black text-white tracking-tight">Insufficient Balance</h3>
                <p className="text-sm text-slate-300 leading-relaxed">
                  Your withdrawal of{' '}
                  <span className="font-bold text-red-400">₦{amount.toLocaleString()}</span>{' '}
                  exceeds your available balance of{' '}
                  <span className="font-bold text-[#029FFC]">₦{user.walletBalance.toLocaleString()}</span>.
                </p>
                <p className="text-xs text-slate-400 leading-relaxed pt-1">
                  Please reduce your withdrawal amount or complete more tasks to earn additional funds.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2.5 pt-1">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 px-4 bg-[#011438] border border-[#025BE5]/40 hover:border-[#029FFC]/60 text-slate-300 hover:text-white font-bold rounded-xl text-xs transition-all flex items-center justify-center gap-1.5"
                >
                  <TrendingUp className="w-3.5 h-3.5 text-[#029FFC]" />
                  Explore Tasks
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  ...
                  <span className="font-black text-[#029FFC]">₦{user.walletBalance.toLocaleString()}</span>
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-1">
              ...
              {!bankVerified && !invalidDetailsError && (
                <p className="text-[11px] text-slate-500 italic">
                  Enter your 10-digit NUBAN and tap <strong className="text-slate-400">Verify</strong> to confirm your account name.
                </p>
              )}

              {/* Resolved Account Name Banner */}
              {bankVerified && !invalidDetailsError && accountName && (
                <div className="p-2.5 bg-[#011438] border border-emerald-500/30 rounded-xl flex items-center gap-2 text-xs animate-in fade-in duration-200">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                  <span className="text-slate-300">
                    Verified Account Name:{' '}
                    <strong className="text-white font-bold">{accountName}</strong>
                  </span>
                </div>
              )}
